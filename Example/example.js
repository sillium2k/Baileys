...
const sendToWebhook = async (webhookUrl, linkData) => {
    try {
    ...
    async (events) => {
        ...
        if (events['connection.update']) {
            ...
            if (connection === 'open' && sendWAMExample) {
                /// sending WAM EXAMPLE
                const { header: { wamVersion, eventSequenceNumber, }, events, } = JSON.parse(await fs.promises.readFile("./boot_analytics_test.json", "utf-8"));
